...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        role_menu, created_role_menu = Menu.objects.get_or_create(parent_id=root.menu_id, path='role', menu_type='C', defaults=role_defaults)
        if not created_role_menu:
            for k, v in role_defaults.items():
                setattr(role_menu, k, getattr(role_menu, k) or v)
            role_menu.visible = '0'
            role_menu.status = '0'
            role_menu.del_flag = '0'
        ...
        post_menu, created_post_menu = Menu.objects.get_or_create(parent_id=root.menu_id, path='post', menu_type='C', defaults=post_defaults)
        if not created_post_menu:
            for k, v in post_defaults.items():
                setattr(post_menu, k, getattr(post_menu, k) or v)
            post_menu.visible = '0'
            post_menu.status = '0'
            post_menu.del_flag = '0'
            post_menu.update_time = now
            post_menu.save()
        ...
        notice_menu, created_notice_menu = Menu.objects.get_or_create(parent_id=root.menu_id, path='notice', menu_type='C', defaults=notice_defaults)
        if not created_notice_menu:
            for k, v in notice_defaults.items():
                setattr(notice_menu, k, getattr(notice_menu, k) or v)
            notice_menu.visible = '0'
            notice_menu.status = '0'
            notice_menu.del_flag = '0'
            notice_menu.update_time = now
            notice_menu.save()
